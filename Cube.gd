...
var center_cube_piece: MeshInstance3D
...
func create_cube() -> void:
	...
	for x in range(3):
		for y in range(3):
			for z in range(3):
				var instance = cube_piece_pref.instantiate()  # Создание экземпляра из префаба
	...
	center_cube_piece = all_cube_pieces[13]
...
func rotate_layer(pieces: Array[MeshInstance3D], rotation_vector: Vector3, 
	...
	var center: Vector3 = center_cube_piece.transform.origin
	...
	while total_angle < 90:
		for piece in pieces:
			...
			var relative_position: Vector3 = piece.transform.origin - center
		...
		total_angle += rotate_angle  # Увеличиваем общий угол для следующего шага
...
func get_up_horizontal_pieces() -> Array[MeshInstance3D]:
	var result: Array[MeshInstance3D] = []
	for cube in all_cube_pieces:
		if round(cube.transform.origin.x) == -1:
			result.append(cube)
	return result
...
func get_up_vertical_pieces() -> Array[MeshInstance3D]:
	var result: Array[MeshInstance3D] = []
	for cube in all_cube_pieces:
		if round(cube.transform.origin.z) == 1:
			result.append(cube)
	return result
...
func get_front_horizontal_pieces() -> Array[MeshInstance3D]:
	var result: Array[MeshInstance3D] = []
	for cube in all_cube_pieces:
		if round(cube.transform.origin.y) == -1:
			result.append(cube)
	return result
...
func detect_left_middle_right_direction(pieces: Array) -> int:
	...
	if pos1.y != pos0.y:
		if pos0.x == -2:
			direction = pos0.y - pos1.y
		else:
			direction = pos1.y - pos0.y
	else:
		if pos0.y == -2:
			direction = pos1.x - pos0.x
		else:
			direction = pos0.x - pos1.x
...
func detect_front_middle_back_direction(pieces: Array) -> int:
	...
	if pos1.z != pos0.z:
		if pos0.y == 0:
			direction = pos1.z - pos0.z
		else:
			direction = pos0.z - pos1.z
	else:
		if pos0.z == 0:
			direction = pos1.y - pos0.y
		else:
